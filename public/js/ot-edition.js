$(document).ready(function () {
    ...
    if (role == 6) { //Rol de Dibujante Técnico (Diseño Estructural) No puede modificar ningun campo de COLOR-CERA-BARNIZ y tampoco el FSC y la planta objetivo
        ...
        if (id_solicitud == '3') { // Muestra con CAD
            $(` #fsc,#impresion,#percentage_coverage_external,#percentage_coverage_internal,
                ...
                #cm2_clisse_color_1,#cm2_clisse_color_2,#cm2_clisse_color_3,#cm2_clisse_color_4,#cm2_clisse_color_5,#cm2_clisse_color_6,#cm2_clisse_color_7`)
                .prop("disabled", true)
                .selectpicker("refresh")
    ...
    $("#planta_id")
        .change(async () => {
            ...
            if ($("#planta_id").val() != '') {
                ...
                //Cambios en datos de Secuencias Operacionales
    ...
    //Verificamos las opciones del carton disponibles para la planta, la impresion y el color del carton seleccionada
    function setListaCartonEdit(color, planta, carton_val, impresion) {
        ...
        return $.ajax({
            ...
            success: function (data) {
                ...
                if ($('#tipo_solicitud_2').val() == 1 || $('#tipo_solicitud_2').val() == 7) {
                    if (role == 6) {
                        $("#carton_id")
                            .prop("disabled", false)
                            .empty()
                ...
                if ($('#impresion').val() === '1') { // 1 => "Offset"
                    ...
                    if (role === '4' || role === '19') {//para el vendedor
                        ...
                        $("#design_type_id,#complejidad,#numero_colores,#color_1_id,#impresion_1,#color_2_id,#impresion_2,#color_3_id,#impresion_3,#color_4_id,#impresion_4,#color_5_id,#impresion_5,#color_6_id,#impresion_6,#color_interno,#impresion_color_interno,#indicador_facturacion_diseno_grafico,#cm2_clisse_color_1,#cm2_clisse_color_2,#cm2_clisse_color_3,#cm2_clisse_color_4,#cm2_clisse_color_5,#cm2_clisse_color_6,#cm2_clisse_color_7")
                            ...
                            .closest("div.form-group")
                            .removeClass("error");
                    } else if (role == 8 || role == 7) {
                        $("#complejidad,#indicador_facturacion_diseno_grafico")
                            .prop("disabled", true)
                            .val("")
                            .selectpicker("refresh")
                            .closest("div.form-group")
    ...
    const num_colores = $('#numero_colores').val();
        ...
        if (role == 4) {
            ...
            $('#trazabilidad').change(function () {
                if (($(this).val() == 2 || $(this).val() == 3) && ($('#impresion').val() == 5)) {
                    $('#numero_colores').val(1).prop('disabled', true).selectpicker('refresh').trigger('change');
                    console.log('entro');
                } else {
                    $('#numero_colores').val(num_colores).prop('disabled', false).selectpicker('refresh').trigger('change');
                }
            });
    ...
    // if (role != 4 && role != 3 && role != 19 && role != 18) { //ANTERIOR
    if (role != 5 && role != 10 && role != 18) { //NUEVOS CAMBIOS, SOLO PUEDE EDITAR CATALOGADOR Y D.E.
        $("#restriccion_pallet,#tamano_pallet_type_id,#altura_pallet,#permite_sobresalir_carga,pais_id,#pallet_qa_id,#bulto_zunchado,#formato_etiqueta,#etiquetas_pallet")
            .prop("disabled", true)
            .selectpicker("refresh");
    } else {
        $("restriccion_pallet")
    ...
    if (role != 18) {
        $('#termocontraible').prop('disabled', true).selectpicker('refresh');
    }
    ...
    setListaCartonEdit($("#carton_color").val(), $("#planta_id").val(), carton_val, $("#impresion").val())
