$(document).ready(function () {
    ...
    $("#form-carga-consumo-energia").on("submit", function (e) {
        ...
        return $.ajax({
            type: "POST",
            url: "/mantenedores/cotizador/consumo_energia/uploading",
            data: formulario,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                let consumo_adhesivos_actualizados =
                    data.consumo_energia_actualizados;
                let consumo_adhesivos_erroneos = data.consumo_energia_error;
               ...
               <td  colspan="4">Consumo Energia con Errores</td>
               </tr>`;
    ...
    $("#archivo").on("click", function (e) {
        ...
        // $("#btn-cargar-consumo-energia").prop("disabled", false);
    });
    $("#archivo").on("change", function (e) {
        if ($(this).get(0).files.length === 0) {
            console.log("No files selected.");
            $("#btn-cargar-consumo-energia").prop("disabled", true);
        } else {
            $("#btn-cargar-consumo-energia").prop("disabled", false);
        }
    });
