$(document).ready(function () {
    ...
    $("#form-carga-consumo-adhesivos-pegados").on("submit", function (e) {
        ...
        return $.ajax({
            type: "POST",
            url: "/mantenedores/cotizador/consumo_adhesivos_pegados/uploading",
            data: formulario,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                $("#btn-cargar-consumo-adhesivos-pegados")
                ...
                // #faffa8 amarillo
                let consumo_adhesivos_pegados_nuevos =
                    data.consumo_adhesivos_pegados;
                let consumo_adhesivos_pegados_actualizados =
                    data.consumo_adhesivos_pegados_actualizados;
                let consumo_adhesivos_pegados_inactivados =
                    data.consumo_adhesivos_pegados_inactivados;
                let consumo_adhesivos_pegados_erroneos =
                    data.consumo_adhesivos_pegados_error;

                let count_consumo_adhesivos_pegados_nuevos = 0;
                let count_consumo_adhesivos_pegados_actualizados = 0;
                let count_consumo_adhesivos_pegados_inactivados = 0;
                let count_consumo_adhesivos_pegados_erroneos = 0;

                let idsConsumoAdhesivosActualizdos =
                    consumo_adhesivos_pegados_actualizados.map(function (
                        consumoAdhesivo
                    ) {
                        count_consumo_adhesivos_pegados_actualizados++;
               ...
               <td  colspan="4">ConsumoAdhesivos con Errores</td>
               </tr>`;
                listadoConsumoAdhesivosErroneos +=
                    consumo_adhesivos_pegados_erroneos
                        .map(function (consumoAdhesivo) {
                            count_consumo_adhesivos_pegados_erroneos++;
                            return `<tr style="background-color:#eac7c7;">
               
               <td colspan="1">Linea Excel #${consumoAdhesivo.linea}</td>
               <td colspan="3">${consumoAdhesivo.motivos}</td>
               
               </tr>`;
                        })
                        .join("");
                if (consumo_adhesivos_pegados_erroneos.length > 0) {
    ...
    $("#archivo").on("change", function (e) {
        if ($(this).get(0).files.length === 0) {
            ...
            $("#btn-cargar-consumo-adhesivos-pegados").prop("disabled", true);
        } else {
            $("#btn-cargar-consumo-adhesivos-pegados").prop("disabled", false);
        }
    });
});
