$(document).ready(function () {
    ...
    $("#form-carga-insumos-paletizados").on("submit", function (e) {
        ...
        return $.ajax({
            type: "POST",
            url: "/mantenedores/cotizador/insumos_paletizados/uploading",
            data: formulario,
            processData: false,
            contentType: false,
            cache: false,
            success: function (data) {
                ...
                $("#btn-cargar-insumos-paletizados")
                ...
                // #faffa8 amarillo
                let paletizados_actualizados =
                    data.insumoPaletizados_actualizados;
                let paletizados_erroneos = data.insumoPaletizados_error;
                ...
                let idsPaletizadosActualizdos = paletizados_actualizados.map(
                    function (paletizado) {
                        ...
                        $("#insumo-paletizado-row-" + paletizado.id).css(
                            "background-color",
                            "#faffa8"
                        );
               ...
               <td  colspan="3">Paletizados con Errores</td>
               </tr>`;
               ...
               <td colspan="1">Linea Excel #${paletizado.linea}</td>
               <td colspan="2">${paletizado.motivos}</td>
               
               </tr>`;
                    })
                    .join("");
                if (paletizados_erroneos.length > 0) {
                    $("#listadoInsumosPaletizadoMasivos").append(
                        listadoPaletizadosErroneos
                    );
                }

                $("#insumosPaletizadosActualizados").html(
                    count_paletizados_actualizados
                );
                $("#insumosPaletizadosErroneos").html(
                    count_paletizados_erroneos
                );
            },
            error: function (err) {
    ...
    $("#archivo").on("change", function (e) {
        if ($(this).get(0).files.length === 0) {
            ...
            $("#btn-cargar-insumos-paletizados").prop("disabled", true);
        } else {
            $("#btn-cargar-insumos-paletizados").prop("disabled", false);
        }
    });
});
