$(document).ready(function () {
    ...
    $('#hierarchy_id').val($('#jerarquia1').val()).selectpicker('refresh');
    ...
    if (role == 6) { //Rol de Dibujante Técnico (Diseño Estructural) No puede modificar ningun campo de COLOR-CERA-BARNIZ y tampoco el FSC y la planta objetivo
        $("#fsc,#planta_id,#impresion,#coverage_external_id,#percentage_coverage_external,#coverage_internal_id,#percentage_coverage_internal,#design_type_id,#barniz_uv,#porcentanje_barniz_uv,#complejidad,#numero_colores,#color_1_id,#impresion_1,#color_2_id,#impresion_2,#color_3_id,#impresion_3,#color_4_id,#impresion_4,#color_5_id,#impresion_5,#color_6_id,#impresion_6,#color_interno,#impresion_color_interno,#indicador_facturacion_diseno_grafico,#cm2_clisse_color_1,#cm2_clisse_color_2,#cm2_clisse_color_3,#cm2_clisse_color_4,#cm2_clisse_color_5,#cm2_clisse_color_6,#cm2_clisse_color_7")
            .prop("disabled", true)
            .selectpicker("refresh")
    ...
    const functionDesignType = () => {
        ...
        if (role == 4 || role == 6 || role == 7 || role == 8 || role == 19) {
    ...
    let plantaAnterior = $("#planta_id").val();

    $("#planta_id")
        .change(() => {

            // if (role == 18) {
            var val = $("#planta_id").val();

            // Mostrar mensaje de confirmación
            var confirmacion = confirm("Al cambiar de planta de eliminaran las secuencias operacionales y se deberan ingresar de nuevo. ¿Estás seguro de cambiar la planta?.");
            ...
            if (confirmacion) {
                ...
                $("#planta_id").val(plantaAnterior);
                $('#planta_id').selectpicker("refresh");
            }
            // }
    ...
    //Funcion para cuando carga
    const functionPlanta = () => {

        let impresion = $("#impresion").val();
    ...
    const setPlanta = (val) => {
        return $.ajax({
            type: "GET",
            url: "/getPlantaCarton",
            data: "planta_id=" + val + "&select_planta_id=" + $("#planta_id").val(),
            success: function (data) {
                $("#planta_id")
                    .empty()
                    .append(data)
    ...
    // ------------- RECUBRIMIENTO ----------
    //Se desabilitan la opcion Cera , ya que son reemplazadas solo por los valores activos en la tabla recubrimiento_type
    ...
    //         $("#porcentaje_cera_exterior,#porcentaje_cera_interior,#porcentaje_barniz_interior")
    //             .prop("disabled", true)
    //             .val("")
    //             .selectpicker("refresh")
    //             .closest("div.form-group")
    ...
    //         $("#porcentaje_cera_exterior,#porcentaje_cera_interior")
    //         .prop("disabled", true)
    //         .val("")
    //         .selectpicker("refresh")
    //         .closest("div.form-group")
    ...
    //          $("#porcentaje_cera_exterior,#porcentaje_barniz_interior")
    //          .prop("disabled", true)
    //          .val("")
    //          .selectpicker("refresh")
    //          .closest("div.form-group")
    ...
    //          $("#porcentaje_cera_interior,#porcentaje_barniz_interior")
    //          .prop("disabled", true)
    //          .val("")
    //          .selectpicker("refresh")
    //          .closest("div.form-group")
        ...
        .change(() => {
            ...
            if (numeroColores === "0" || numeroColores === "") {
                if ($("#coverage_external_id").val() != 4) {
                    ...
                    if (role != 3 && role != 4 && role != 5 && role != 6 && role != 18 && role != 19) {
                        $("#barniz_uv,#porcentanje_barniz_uv")
                            .prop("disabled", false)
                            .prop('required', true)
                            .selectpicker("refresh")
    ...
    if ($("#tipo_solicitud").val() != 1 && $("#tipo_solicitud").val() != 5) {
        $("#reference_type,#reference_id,#bloqueo_referencia,#cinta")
            .prop("disabled", true)
            //.val("")
            .selectpicker("refresh");
    }
    ...
    $("#cad_id").change(function () {
        ...
        return $.ajax({
            ...
            success: function (data) {
                ...
                //Se inahibilita el set Rayados segun correo y confirmacion cliente para mostrar datos de rayados de la OT
                // y del CAD Asignado a la misma
                //Correo asunto: Fwd: OT19781 Agregar pegado fecha 23/04/2024
                //setRayados(data);
            },
        });
    });
    ...
    const tipo_solicitud = $("#tipo_solicitud").val();
    ...
    if (tipo_solicitud == 1) {
        ...
        if (role == 5 || role == 6 || role == 18) {
            ...
            if (cad_asignado) {
                ...
                if (role == 18) {
                    ...
                    // CAMBIAR SELECTOR MATRIZ SEGUN CAD SELECCIONADO
        ...
        if (role == 18) {
            $("#reference_type,#product_type_id").prop("disabled", false);
        } else {
            $("#cad_id,#reference_type,#product_type_id").prop("disabled", true);
        }
        let material_asignado = $("#material_asignado").val();
        if (material_asignado) {
    ...
    if (role == 8 || role == 7) {
        ...
        //Validacion para hibilitar modificacion campo tipo de diseño para el rol Diseñador
    ...
    if ($("#material_asignado").val() == '') {
        if (role == 3 || role == 4 || role == 5 || role == 6 || role == 19) {//Validar roles vendedores
        ...
        if (role == 18) {
            ...
            $("#product_type_developing_id,#food_type_id,#expected_use_id,#recycled_use_id,#class_substance_packed_id,#transportation_way_id,#target_market_id")
                .prop("disabled", true)
                .prop("readonly", true)
                .selectpicker("refresh")
                .closest("div.form-group")
    ...
    function setListaCartonEdit(color, planta, carton_val) {


        return $.ajax({
            type: "GET",
            url: "/getListaCartonEdit",
            data: "carton_color=" + color + "&planta=" + planta,
            success: function (data) {
                //data = $.parseHTML(data);

                console.log($('#tipo_solicitud_select_ppal').val());
                $("#carton_id")
                    .prop("disabled", false)
                    .empty()
                    .append(data)
    ...
    var carton_val = $("#carton_id").val();
    setListaCartonEdit($("#carton_color").val(), $("#planta_id").val(), carton_val);
    ...
    if (role == 18) {
        $("#descripcion_material").prop("readonly", false);
    }

    if (role == 5 || role == 6) {
        $("#reference_id,#bloqueo_referencia,#product_type_id,#carton_color,#target_market_id").prop("disabled", true).selectpicker("refresh");
    }
    ...
    getMatrizValue($('#ot_id').val());
    // $('#matriz_id').prop('disabled',true);

    //Evolutivo 24-11 ajuste 2 - Inicio
    // if (role != 3 && role != 4 && role != 18 && role != 19){
    //     $('#trazabilidad').prop('disabled', true);
    // }else{
...
$("#modal-log-mckee").on("show.bs.modal", function (e) {
    var val = $('#ot_id').val();
    var html = ''
    $("#tabla_log_mckee tbody").html(html);
    return $.ajax({
        type: "GET",
        url: "/detalleMckee",
        data: "ot_id=" + val,
...
function chargeSelectSecOperacional(planta_id, ot_id) {
...
function getSecuenciasOperacionalesOt(ot_id) {

    console.log(ot_id);
    return $.ajax({
        type: "GET",
        url: "/getSecuenciasOperacionalesOt",
        data: "ot_id=" + ot_id,
        success: function (data) {
            ...
            $('#sec_operacional_principal').val(data.so_planta_original);
            $('#sec_operacional_1').val(data.so_planta_alt1);
            $('#sec_operacional_2').val(data.so_planta_alt2);
...
function chargeSelectSecOperacionalPlanta(ot_id) {
    ...
    return $.ajax({
        ...
        success: function (data) {
            ...
            console.log('AAAAAA:' + data.planta);
            if (data.planta === null || typeof data.planta === 'undefined') {
                if ($('#planta_id').val() == 1) {
                    $('#planta_original_sec_ope').val('BUIN');
                    getSecuenciasOperacionalesPlanta(1);
                    $('#sec_ope_planta_orig_id').val(1);

                } else {
                    if ($('#planta_id').val() == 2) {
                        $('#planta_original_sec_ope').val('TILTIL');
                        getSecuenciasOperacionalesPlanta(2);
                        $('#sec_ope_planta_orig_id').val(2);
                    } else {
                        $('#planta_original_sec_ope').val('OSORNO');
                        getSecuenciasOperacionalesPlanta(3);
                        $('#sec_ope_planta_orig_id').val(3);
                    }
                }
...
function chargeSelectSecOperacionalPlantaAux1(ot_id) {
    ...
    return $.ajax({
        ...
        success: function (data) {
            ...
            if (data.planta === null || typeof data.planta === 'undefined') {
                if ($('#planta_id').val() == 1) {
                    $('#planta_aux_1_sec_ope').val('TILTIL');
                    getSecuenciasOperacionalesPlantaAux1(2);
                    $('#sec_ope_planta_aux_1_id').val(2);
                } else {
                    if ($('#planta_id').val() == 2) {
                        $('#planta_aux_1_sec_ope').val('BUIN');
                        getSecuenciasOperacionalesPlantaAux1(1);
                        $('#sec_ope_planta_aux_1_id').val(1);
                    } else {
                        $('#planta_aux_1_sec_ope').val('BUIN');
                ...
                if (data.cantidad_filas > 6) {
                    ...
                    if (data.planta == 1) {
                        ...
                        if (data.planta == 2) {
                            ...
                            $('#planta_aux_1_sec_ope').val('OSORNO');
...
function chargeSelectSecOperacionalPlantaAux2(ot_id) {
    ...
    return $.ajax({
        ...
        success: function (data) {
            ...
            if (data.planta === null || typeof data.planta === 'undefined') {
                if ($('#planta_id').val() == 1) {
                    $('#planta_aux_2_sec_ope').val('OSORNO');
                    getSecuenciasOperacionalesPlantaAux2(3);
                    $('#sec_ope_planta_aux_2_id').val(3);
                } else {
                    if ($('#planta_id').val() == 2) {
                        ...
                        $('#planta_aux_2_sec_ope').val('TILTIL');
                        getSecuenciasOperacionalesPlantaAux2(2);
                        $('#sec_ope_planta_aux_2_id').val(2);
                    }
                }
...
$('#sec_ope_ppal_planta_ori_1,#sec_ope_atl_1_planta_ori_1,#sec_ope_atl_2_planta_ori_1,#sec_ope_atl_3_planta_ori_1,#sec_ope_atl_4_planta_ori_1,#sec_ope_atl_5_planta_ori_1,#sec_ope_ppal_planta_ori_2,#sec_ope_atl_1_planta_ori_2,#sec_ope_atl_2_planta_ori_2,#sec_ope_atl_3_planta_ori_2,#sec_ope_atl_4_planta_ori_2,#sec_ope_atl_5_planta_ori_2,#sec_ope_ppal_planta_ori_3,#sec_ope_atl_1_planta_ori_3,#sec_ope_atl_2_planta_ori_3,#sec_ope_atl_3_planta_ori_3,#sec_ope_atl_4_planta_ori_3,#sec_ope_atl_5_planta_ori_3,#sec_ope_ppal_planta_ori_4,#sec_ope_atl_1_planta_ori_4,#sec_ope_atl_2_planta_ori_4,#sec_ope_atl_3_planta_ori_4,#sec_ope_atl_4_planta_ori_4,#sec_ope_atl_5_planta_ori_4,#sec_ope_ppal_planta_ori_5,#sec_ope_atl_1_planta_ori_5,#sec_ope_atl_2_planta_ori_5,#sec_ope_atl_3_planta_ori_5,#sec_ope_atl_4_planta_ori_5,#sec_ope_atl_5_planta_ori_5,#sec_ope_ppal_planta_ori_6,#sec_ope_atl_1_planta_ori_6,#sec_ope_atl_2_planta_ori_6,#sec_ope_atl_3_planta_ori_6,#sec_ope_atl_4_planta_ori_6,#sec_ope_atl_5_planta_ori_6')
    .prop('disabled', true)
    .selectpicker('refresh');
$('#sec_ope_ppal_planta_aux_1_1,#sec_ope_atl_1_planta_aux_1_1,#sec_ope_atl_2_planta_aux_1_1,#sec_ope_ppal_planta_aux_1_2,#sec_ope_atl_1_planta_aux_1_2,#sec_ope_atl_2_planta_aux_1_2,#sec_ope_ppal_planta_aux_1_3,#sec_ope_atl_1_planta_aux_1_3,#sec_ope_atl_2_planta_aux_1_3,#agregar_fila_planta_aux_1')
    .prop('disabled', true)
    .selectpicker('refresh');
$('#sec_ope_ppal_planta_aux_2_1,#sec_ope_atl_1_planta_aux_2_1,#sec_ope_atl_2_planta_aux_2_1,#sec_ope_ppal_planta_aux_2_2,#sec_ope_atl_1_planta_aux_2_2,#sec_ope_atl_2_planta_aux_2_2,#sec_ope_ppal_planta_aux_2_3,#sec_ope_atl_1_planta_aux_2_3,#sec_ope_atl_2_planta_aux_2_3,#agregar_fila_planta_aux_2')
    .prop('disabled', true)
    .selectpicker('refresh');

$('#planta_original_sec_ope,#planta_aux_1_sec_ope,#planta_aux_2_sec_ope').prop('readonly', true).prop('disabled', true);
...
function getMatriz(val) {
    ...
    return $.ajax({
        ...
        success: function (data) {
            ...
            $('#matriz_id').val('').selectpicker("refresh");
...
function getMatrizValue(ot_id) {
    console.log('GETMATRIZvalue');
    return $.ajax({
        type: "GET",
        url: "/getOtData",
        data: "ot_id=" + ot_id,
        success: function (data) {
            ...
            $('#matriz_id').val(data.matriz_id).selectpicker("refresh");
            $('#tipo_matriz_text').val(data.tipo_matriz);
        },
    });
}
