...
/*
        ...
        $("#coverage_type_id")
            .html(
                `<option value="">Seleccionar...</option>
                <option value="4"> Barniz: Acuoso </option>
                <option value="5"> Barniz: UV Brillante </option>`
            )
            .selectpicker("refresh");
    ...
    // si proceso es flexo = 1 o sin proceso = 3 no lleva matriz bloquear "NO"
    // if (proceso.val() == 1 || proceso.val() == 3) {
    //     matriz
    //         .prop("disabled", true)
    //         .prop("readonly", true)
    //         .selectpicker("val", 0)
...
numero_colores.on("change", function () {
    ...
    if (numero_colores.val() && numero_colores.val() != "0") {
        clisse
            .prop("disabled", false)
            .prop("readonly", false)
            .selectpicker("refresh");
...
$(document).on(
    ...
    ".calcular-margen,.margen-detalle"
);
...
function defaultValues() {
   ...
   // $("#porcentaje_cera_interno,#porcentaje_cera_externo,#coverage_type_id").prop("disabled", true);
    ...
    // Selects
    $(
        "#devolucion_pallets,#tipo_medida,#pallet,#zuncho,#stretch_film"
    ).selectpicker("val", 1);
    $(
        "#ajuste_precios,#clisse,#armado_automatico,#maquila,#royalty,#funda,#cinta_desgarro,#impresion"
    ).selectpicker("val", 0);
    // if(!window.impresion){
...
function renderTablasResultados() {
    // debugger;
    var listadoResultadosDetalle, listadoCostosDetalle, listadoCostosServicios;
    ...
    window.detalles_cotizaciones.map(function (detalle, index) {
        ...
        if (
            ...
            margen = `<input class="text-center margen-detalle"style="width:80%" type="text" id="margen_detalle_${
                detalle.id
            }" maxlength="7" name="margen_detalle_${
                detalle.id
            }" value="${separadorMiles(
                parseFloat(detalle.margen).toFixed(1)
            )}">`;
            precio_total_usd_mm2 = `<input class="text-center calcular-margen"style="width:80%" type="text" readonly="true" id="precio_total_usd_mm2_${
                detalle.id
            }"name="precio_total_usd_mm2_${
                detalle.id
            }" value="${separadorMiles(
                parseFloat(detalle.precios.precio_total.usd_mm2).toFixed(1)
            )}">`;
            precio_total_usd_caja = `<input class="text-center calcular-margen"style="width:80%" type="text" readonly="true" id="precio_total_usd_caja_${
                detalle.id
            }" name="precio_total_usd_caja_${
                detalle.id
            }" value="${redondeo(detalle.precios.precio_total.usd_caja)}">`;
            precio_total_clp_caja = `<input class="text-center calcular-margen"style="width:80%" type="text" readonly="true" id="precio_total_clp_caja_${
                detalle.id
            }" name="precio_total_clp_caja_${
                detalle.id
            }" value="${separadorMiles(
        ...
        switch (tipo_detalle) {
            ...
            case 1:
                ...
                if (
                    ...
                    }else if( detalle.print_type_id == 2 ){
        ...
        let precio_caja = redondeo((detalle.precios.precio_total.usd_caja * detalle.cantidad) / 1000);
        ...
        let parseValorMm2 = parseFloat(valor_mm2.replace(/,/g, '.'));
        
        //Datos para calcular el valor ton para el calculo final 
        let precio_ton = separadorMiles(parseFloat(detalle.precios.precio_total.usd_ton).toFixed(1));
        ...
        let parseValorTon = parseFloat(valor_ton.replace(/,/g, '.'));
        ...
        let precio_usd_un_valor = redondeo(detalle.precios.precio_total.usd_caja);
    ...
    if (
        margen_sugerido_total <= margen_ingresado_total ||
        (window.user.role_id == 3 &&
            margen_sugerido_total - margen_sugerido_total * 0.1 <=
                margen_ingresado_total)
    ) {
...
//Solo se activa esta validacion si es con el metodo de Cotizar multiples OT , ya que se tienen que completar unos datos
$("#generarPrecotizacion").on('click', function(e) {
    ...
    if (!!dids) {
        window.detalles_cotizaciones.forEach((element) => {
            if(element.ciudad_id === null){   
                bandera = true;
            }
        });
    }
    if(bandera){
        $("#listado-detalles").css({"border": "solid 2px red"});
        notify("Complete los datos del detalle", "danger");
    }else{

        $("#listado-detalles").css({"border": ""});
    }
