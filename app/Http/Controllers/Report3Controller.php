...
class Report3Controller extends Controller
{
    ...
    public function reportGestionLoadOtMonth()
    {
        ...
        return view('reports3.reportGestionLoadOtMonth', compact(
            'vendedores',
            'clients',
            'estados',
            'mes',
    ...
    public function reportCompletedOtEntreFechas()
    {
        ...
        return view(
            'reports3.reportCompletedOtEntreFechas',
            compact(
                'fromDate',
                'toDate',
                'vendedores',
    ...
    public function reportTimeByAreaOtMonth()
    {
        ...
        return view(
            'reports3.reportTimeByAreaOtMonth',
            compact(
                'vendedores',
                'clients',
                'mes',
    ...
    public function reportGestionOtActives()
    {
        ...
        return view('reports3.reportGestionOtActives', compact('top_responsables', 'vendedores', 'canals', 'solicitudesTotales', 'solicitudesPorArea', 'porcentajeSolicitudesPorArea', 'totalDesarrolloPorArea', 'desarrollosPorArea', 'desarrollosPorAreaDias', 'totalArtePorArea', 'artesPorArea', 'artesPorAreaDias', 'totalCotizaConCadPorArea', 'totalCotizaSinCadPorArea', 'totalMuestraConCadPorArea', 'cotizaConCadPorArea', 'cotizaConCadPorAreaDias', 'cotizaSinCadPorArea', 'cotizaSinCadPorAreaDias', 'muestraConCadPorArea', 'muestraConCadPorAreaDias', 'solicitudesPorTipoSolicitudPorArea', 'solicitudesPorTipoSolicitudPorAreaDias', 'estados', "cantidadPorEstado", "tiempoPromedioPorEstado", "responsables"));
    }
    ...
    public function reportReasonsRejectionMonth()
    {
        ...
        return view('reports3.reportReasonsRejectionMonth', compact(
            'fromDate',
            'toDate',
            'vendedores',
            'clients',
    ...
    public function reportRechazosPorMes()
    {
        ...
        return view('reports3.reportRechazosPorMes', compact('vendedores', 'mes', 'years', 'meses', 'solicitudesTotalesUltimosMeses', "nombreMeses", "nombreMesesSeleccionados", "faltaInformacion", "informacionErronea", "faltaMuestraFisica", "formatoImagenInadecuado", "medidaErronea", "descripcionDeProducto", "planoMalAcotado", "errorDeDigitacion", "errorTipoSustrato", "noViablePorRestricciones", "faltaCadParaCorte", "faltaOTChileexpress", "faltaOTLaboratorio"));
    }

    public function index()
    {
        // $ot
        return view('reports3.index');
    }
    ...
    public function reporte1()
    {
        ...
        $toDate = Carbon::now()->format('d/m/Y');
        return view('reports3.reporte1', compact('fromDate', 'toDate', "topOt", "topDesarrolloCompleto", "topOtrosDesarrollos"));
    }
    ...
    public function reportActiveOtsPerArea()
    {
        ...
        $count = 0;
        return view(
            'reports3.reportActiveOtsPerArea',
            compact(
                "area_actual",
                "users_actual",
                "areas",
    ...
    public function reportIndicadorSalaMuestra()
    {
        ...
        return view(
            'reports3.reportIndicadorSalaMuestra',
            compact(
                'mes',
                'years',
                'mesSeleccionado',
    ...
    public function reportDisenoEstructuralySalaMuestra()
    {
        ...
        */
        return view(
            'reports3.reportDisenoEstructuralySalaMuestra',
            compact(
                'mes',
                'years',
                'year',
    ...
    public function reportTiempoPrimeraMuestra()
    {
        ...
        return view(
            'reports3.reportTiempoPrimeraMuestra',
            compact(
                'mes',
                'years',
                'mesSeleccionado',
    ...
    public function reportTiempoDisenadorExterno()
    {
        ...
        return view(
            'reports3.reportTiempoDisenadorExterno',
            compact(
                'mes',
                'years',
                'mesSeleccionado',
    ...
    //         "muestrasPrioritarias"
    //     )
    //         ->join('work_spaces', 'work_spaces.id', '=', 'work_orders.current_area_id')
    //         ->join('managements', 'work_orders.id', '=', 'managements.work_order_id')
    //         ->where('work_orders.current_area_id', '!=', 1)
    ...
    //             DB::raw('COUNT(*) as cantidad'),
    //             DB::raw('MAX(work_orders.id) as last_id') // para ordenar por último id
    //         )
    //         ->groupBy(
    //             'area_nombre',
    //             'area_abreviatura_nombre',
    //             'current_area_id'
    //         )
    //         ->orderByDesc('last_id') // usamos el alias agregado arriba
    //         ->get();

    //     // Definir el orden y valores por defecto
    //     $array_key_definition = ['DE' => 0, 'SM' => 0, 'DG' => 0, 'PC' => 0, 'C' => 0];

    //     // Llenar valores obtenidos de la consulta
    ...
    public function cantidad_ot_por_area_mes_actual($area_id, $vendedor_id)
{
    ...
    $query = WorkOrder::with(
        ...
        ->join('managements', 'work_orders.id', '=', 'managements.work_order_id')
        ->where('work_orders.current_area_id', '!=', 1)
        ->where('work_orders.id', 24583) // 🔹 filtramos por la OT específica
        ->whereIn('creador_id', $vendedor_id)
        ->whereIn('current_area_id', $area_id)
        ->where('managements.management_type_id', 1)
        ->whereIn("managements.state_id", $estados_activos)
        ...
        ->select(
            ...
            END) AS area_abreviatura_nombre'),
            DB::raw('COUNT(*) as cantidad'),
            DB::raw('MAX(work_orders.id) as last_id')
        )
        ->groupBy(
            'area_nombre',
            'area_abreviatura_nombre',
    ...
    //     $ots = $query
    //         // ->selectRaw('MAX(work_orders.id) as last_id')
    //         ->groupBy(
    //             'area_nombre',
    //             'area_abreviatura_nombre',
    //             'current_area_id'
    ...
    public function calcula_el_promedio($data, $fromDate, $toDate, $vendedorid, $promedio_anio)
    {
        ...
        $dic_ultimoCambioEstado_byWorkOrderId = DB::table('managements')
            ...
            ->whereNotNull('state_id')
            ->where('created_at', '>=', $fromDate)
            ->where('work_order_id', 24583) // Exclude specific work order if needed
            ->orderBy('work_order_id', 'asc')
            ->orderBy('created_at', 'asc')
            ->get()
            ->pluck('state_id', 'work_order_id');
    ...
    public function cantidad_ot_cortadas_terminadas($fromDate, $toDate)
    {
        ...
        $muestras = Muestra::where("work_order_id", 24583)
            //->whereIn("estado", ["1", "3"])
            //->whereBetween('created_at', [$fromDate, $toDate])
            ->where("estado", "3")
            ->whereBetween('ultimo_cambio_estado', [$fromDate, $toDate])
