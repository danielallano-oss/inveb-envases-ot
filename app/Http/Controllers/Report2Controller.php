...
class Report2Controller extends Controller
{
    ...
    public function reportGestionLoadOtMonth()
    {
        ...
        if (!is_null(request()->input('exportar'))) {
            ...
            //         $q->select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))->where('management_type_id', 1);
            //     }
            // ]);
            // if (!is_null(request()->query('vendedor_id'))) {
            //     $query = $query->whereIn('creador_id', request()->query('vendedor_id'));
            ...
            $otsDias = $otsDias->map(function ($ot) {
                $ot->dias_trabajados = round($ot->present()->diasTrabajados2($ot->tiempo_total), 1);
                return $ot;
            });
            ...
            $otsDias = $otsDias->map(function ($ots_por_tipo_solicitud, $new_date) use ($meses, &$diasPorSolicitudUltimosMeses, &$numeroPorSolicitudUltimosMeses, &$muestraPromedioDiasTotalesUltimosMeses, &$muestraPromedioDiasTotalesUltimosMesesCount, &$desarrolloCompletoPromedioDiasTotalesUltimosMeses, &$desarrolloCompletoPromedioDiasTotalesUltimosMesesCount, &$artePromedioDiasTotalesUltimosMeses, &$artePromedioDiasTotalesUltimosMesesCount, &$otrasDesarrolloPromedioDiasTotalesUltimosMeses, &$otrasDesarrolloPromedioDiasTotalesUltimosMesesCount, &$proyectoInnovacionPromedioDiasTotalesUltimosMeses, &$proyectoInnovacionPromedioDiasTotalesUltimosMesesCount) {
                ...
                if ($key !== false) {
                    $ots_por_tipo_solicitud->map(function ($ots, $tipo_solicitud) use ($key, $meses, &$diasPorSolicitudUltimosMeses, &$numeroPorSolicitudUltimosMeses, &$muestraPromedioDiasTotalesUltimosMeses, &$muestraPromedioDiasTotalesUltimosMesesCount, &$desarrolloCompletoPromedioDiasTotalesUltimosMeses, &$desarrolloCompletoPromedioDiasTotalesUltimosMesesCount, &$artePromedioDiasTotalesUltimosMeses, &$artePromedioDiasTotalesUltimosMesesCount, &$otrasDesarrolloPromedioDiasTotalesUltimosMeses, &$otrasDesarrolloPromedioDiasTotalesUltimosMesesCount, &$proyectoInnovacionPromedioDiasTotalesUltimosMeses, &$proyectoInnovacionPromedioDiasTotalesUltimosMesesCount) {
                        $diasTrabajados2 = round($ots->sum("dias_trabajados"), 1);
                        $numeroOts = $ots->count();
                        $diasPorSolicitudUltimosMeses[$key] += $diasTrabajados2;
                        $numeroPorSolicitudUltimosMeses[$key] += $numeroOts;
                        switch ($tipo_solicitud) {
                            case '1':
                                $desarrolloCompletoPromedioDiasTotalesUltimosMeses[$key] += $diasTrabajados2;
                                $desarrolloCompletoPromedioDiasTotalesUltimosMesesCount[$key] += $numeroOts;
                                break;
                            /*case '2':
                                $cotizaConCadPromedioDiasTotalesUltimosMeses[$key] += $diasTrabajados2;
                                $cotizaConCadPromedioDiasTotalesUltimosMesesCount[$key] += $numeroOts;
                                break;*/
                            case '3':
                                $muestraPromedioDiasTotalesUltimosMeses[$key] += $diasTrabajados2;
                                $muestraPromedioDiasTotalesUltimosMesesCount[$key] += $numeroOts;
                                break;
                            /*case '4':
                                $cotizaSinCadPromedioDiasUltimosMeses[$key] += $diasTrabajados2;
                                $cotizaSinCadPromedioDiasUltimosMesesCount[$key] += $numeroOts;
                                break;*/
                            case '5':
                                $artePromedioDiasTotalesUltimosMeses[$key] += $diasTrabajados2;
                                $artePromedioDiasTotalesUltimosMesesCount[$key] += $numeroOts;
                                break;
                            case '6':
                                $otrasDesarrolloPromedioDiasTotalesUltimosMeses[$key] += $diasTrabajados2;
                                $otrasDesarrolloPromedioDiasTotalesUltimosMesesCount[$key] += $numeroOts;
                                break;
                            case '7':
                                $proyectoInnovacionPromedioDiasTotalesUltimosMeses[$key] += $diasTrabajados2;
                    ...
                    // $diasTrabajados2 = round($ot->present()->diasTrabajados2($ot->tiempo_total), 1);
                    // $diasTrabajados2 = round(($ot->tiempo_total / 3600) / 24, 1);
        ...
        return view('reports2.reportGestionLoadOtMonth', compact(
            'vendedores',
            'clients',
            'estados',
            'mes',
        ...
        if (!is_null(request()->input('exportar'))) {
            ...
            $query = $query->withCount([
                'gestiones AS tiempo_venta' => function ($q) {
                    $q->select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))->where('management_type_id', 1)->where('work_space_id', 1);
                }
            ]);
            // Calculo total de tiempo en area de desarrollo
            $query = $query->withCount([
                'gestiones AS tiempo_desarrollo' => function ($q) {
                    $q->select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))->where('management_type_id', 1)->where('work_space_id', 2);
                }
            ]);
            // Calculo total de tiempo en area de diseño
            $query = $query->withCount([
                'gestiones AS tiempo_diseño' => function ($q) {
                    $q->select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))->where('management_type_id', 1)->where('work_space_id', 3);
                }
            ]);
            // Calculo total de tiempo en area de catalogacion
            $query = $query->withCount([
                'gestiones AS tiempo_catalogacion' => function ($q) {
                    $q->select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))->where('management_type_id', 1)->where('work_space_id', 4);
                }
            ]);
            // Calculo total de tiempo en area de precatalogacion
            $query = $query->withCount([
                'gestiones AS tiempo_precatalogacion' => function ($q) {
                    $q->select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))->where('management_type_id', 1)->where('work_space_id', 5);
                }
            ]);
            // Calculo total de tiempo
            $query = $query->withCount([
    ...
    public function reportCompletedOtEntreFechas()
    {
        ...
        return view(
            'reports2.reportCompletedOtEntreFechas',
            compact(
                'fromDate',
                'toDate',
                'vendedores',
    ...
    public function reportTimeByAreaOtMonth()
    {
        ...
        $query = $query->withCount([
            'gestiones AS tiempo_muestra' => function ($q) {
                $q->select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))->where('management_type_id', 1)->where('work_space_id', 6);
            }
        ]);
        // Calculo total de tiempo en area de diseño
        $query = $query->withCount([
        ...
        $otsDias = $otsDias->map(function ($ot) {
            $ot->dias_trabajados = round($ot->present()->diasTrabajados2($ot->tiempo_total), 1);
            $ot->dias_trabajados_venta = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_venta, 1), 1);
            $ot->dias_trabajados_desarrollo = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_desarrollo, 2), 1);
            $ot->dias_trabajados_muestra = round($ot->present()->diasTrabajadosPorArea2SalaMuestra($ot->tiempo_muestra, 6), 1);
            $ot->dias_trabajados_diseño = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_diseño, 3), 1);
            $ot->dias_trabajados_catalogacion = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_catalogacion, 4), 1);
            $ot->dias_trabajados_precatalogacion = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_precatalogacion, 5), 1);
            return $ot;
        });
        // gerardoCode
        ...
        $otsDias = $otsDias->map(function ($ots, $new_date) use ($meses, &$diasPorSolicitudUltimosMeses, &$numeroPorSolicitudUltimosMeses, &$ventaPromedioDiasUltimosMeses, &$ventaPromedioDiasUltimosMesesCount, &$desarrolloPromedioDiasTotalesUltimosMeses, &$desarrolloPromedioDiasTotalesUltimosMesesCount, &$muestrasPromedioDiasTotalesUltimosMeses, &$muestrasPromedioDiasTotalesUltimosMesesCount, &$diseñoPromedioDiasTotalesUltimosMeses, &$diseñoPromedioDiasTotalesUltimosMesesCount, &$catalogacionPromedioDiasTotalesUltimosMeses, &$catalogacionPromedioDiasTotalesUltimosMesesCount, &$precatalogacionPromedioDiasTotalesUltimosMeses, &$precatalogacionPromedioDiasTotalesUltimosMesesCount) {
            ...
            if ($key !== false) {
                $ots->map(function ($ot) use ($key, &$diasPorSolicitudUltimosMeses, &$numeroPorSolicitudUltimosMeses, &$ventaPromedioDiasUltimosMeses, &$ventaPromedioDiasUltimosMesesCount, &$desarrolloPromedioDiasTotalesUltimosMeses, &$desarrolloPromedioDiasTotalesUltimosMesesCount, &$muestrasPromedioDiasTotalesUltimosMeses, &$muestrasPromedioDiasTotalesUltimosMesesCount, &$diseñoPromedioDiasTotalesUltimosMeses, &$diseñoPromedioDiasTotalesUltimosMesesCount, &$catalogacionPromedioDiasTotalesUltimosMeses, &$catalogacionPromedioDiasTotalesUltimosMesesCount, &$precatalogacionPromedioDiasTotalesUltimosMeses, &$precatalogacionPromedioDiasTotalesUltimosMesesCount) {
                    $diasTrabajados2 = round($ot->dias_trabajados, 1);
        ...
        $creadores = $creadores->map(function ($creador) {
            ...
            $creador->tiempo_total = $creador->otsCreadas->sum('tiempo_total') / 3600 / 24;
        ...
        $clientes = $clientes->map(function ($cliente) {
            ...
            $cliente->tiempo_total = $cliente->ots->sum('tiempo_total') / 3600 / 24;
        ...
        return view(
            'reports2.reportTimeByAreaOtMonth',
            compact(
                'vendedores',
                'clients',
                'mes',
    ...
    public function reportGestionOtActives()
    {
        ...
        $otsDias = $otsDias->map(function ($ot) use (&$solicitudesPorArea, &$totalDesarrolloPorArea, &$desarrollosPorArea, &$desarrollosPorAreaDias, &$totalArtePorArea, &$artesPorArea, &$artesPorAreaDias, &$totalCotizaConCadPorArea, &$totalCotizaSinCadPorArea, &$totalMuestraConCadPorArea, &$cotizaConCadPorArea, &$cotizaConCadPorAreaDias, &$cotizaSinCadPorArea, &$cotizaSinCadPorAreaDias, &$muestraConCadPorArea, &$muestraConCadPorAreaDias, $estados, &$cantidadPorEstado, &$tiempoPorEstado, &$responsables) {
            ...
            $ot->dias_trabajados_venta = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_venta, 1), 1);
            $ot->dias_trabajados_desarrollo = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_desarrollo, 2), 1);
            $ot->dias_trabajados_muestra = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_muestra, 6), 1);
            $ot->dias_trabajados_diseño = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_diseño, 3), 1);
            $ot->dias_trabajados_catalogacion = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_catalogacion, 4), 1);
            $ot->dias_trabajados_precatalogacion = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_precatalogacion, 5), 1);
            ...
            if ($ot->current_area_id == 1 && array_key_exists($ot->ultimoCambioEstado->state->id, $estados) && $ot->vendedorAsignado->user->active == 1) {
                ...
                $tiempoPorEstado[$ot->ultimoCambioEstado->state->id] += get_working_hours2($ot->ultimoCambioEstado->created_at, Carbon::now());
                ...
                $responsables[$ot->vendedorAsignado->user_id]->tiempo_venta += get_working_hours2($ot->ultimoCambioEstado->created_at, Carbon::now());
            }
        ...
        $responsables = $responsables->map(function ($responsable) {
            ...
            if ($responsable->total_ots > 0) {
                // para el tiempo de venta se debe pasar de horas a dias dividiendo entre 24 Horas/dia
                $responsable->tiempo_promedio_venta =  round(($responsable->tiempo_venta / 24) / $responsable->total_ots, 1);
        ...
        foreach ($tiempoPromedioPorEstado as $key => $val) {
            if ($tiempoPorEstado[$key] > 0) {
                $tiempoPromedioPorEstado[$key] =  round(($tiempoPorEstado[$key] / 24) / $cantidadPorEstado[$key], 1);
            }
        }
        ...
        return view('reports2.reportGestionOtActives', compact('top_responsables', 'vendedores', 'canals', 'solicitudesTotales', 'solicitudesPorArea', 'porcentajeSolicitudesPorArea', 'totalDesarrolloPorArea', 'desarrollosPorArea', 'desarrollosPorAreaDias', 'totalArtePorArea', 'artesPorArea', 'artesPorAreaDias', 'totalCotizaConCadPorArea', 'totalCotizaSinCadPorArea', 'totalMuestraConCadPorArea', 'cotizaConCadPorArea', 'cotizaConCadPorAreaDias', 'cotizaSinCadPorArea', 'cotizaSinCadPorAreaDias', 'muestraConCadPorArea', 'muestraConCadPorAreaDias', 'solicitudesPorTipoSolicitudPorArea', 'solicitudesPorTipoSolicitudPorAreaDias', 'estados', "cantidadPorEstado", "tiempoPromedioPorEstado", "responsables"));
    }
    ...
    public function calcularSemaforo($ot, &$tipoSolicitudPorArea, &$tipoSolicitudPorAreaDias)
    {
        switch ($ot->current_area_id) {
            ...
            case 99:
                // TIEMPO TOTAL
                // if ($diasTrabajados2 <= 10) {
                //     return "success";
                // } elseif ($diasTrabajados2 > 10 && $diasTrabajados2 <= 19) {
                //     return "warning";
                // } else {
                //     return "danger";
                // }
    ...
    public function reportReasonsRejectionMonth()
    {
            ...
            switch ($rechazo->consulted_work_space_id) {
                case 1: //Venta
                    ...
                    foreach ($rechazo->ot->gestiones as $gestion) {
                        if ($siguienteGestion) {
                            $tiempoRechazo = $gestion->duracion_segundos_24h;
                            break;
                        }
                        ...
                        if ($rechazo->ot->gestiones->last() == $rechazo) {
                            $tiempoRechazo = get_working_hours2($this->ot->ultimo_cambio_area, Carbon::now()) * 3600;
                            break;
                        }
                    ...
                    $responsables[$rechazo->ot->vendedorAsignado->user_id]->tiempo += $tiempoRechazo / 3600 / 24;
                    break;
                case 2: //Dibujo estructural
                    ...
                    // llevar tiempo de segundos a dias
                    $responsables[$rechazo->ot->ingenieroAsignado->user_id]->tiempo += $tiempoRechazo / 3600 / 24;
                    break;
                case 3: //dibujo grafico
                    ...
                    // llevar tiempo de segundos a dias
                    $responsables[$rechazo->ot->diseñadorAsignado->user_id]->tiempo += $tiempoRechazo / 3600 / 24;
                    break;
                    break;
                default:
                    # code...
        ...
        return view('reports2.reportReasonsRejectionMonth', compact(
            'fromDate',
            'toDate',
            'vendedores',
            'clients',
    ...
    public function reportRechazosPorMes()
    {
        ...
        return view('reports2.reportRechazosPorMes', compact('vendedores', 'mes', 'years', 'meses', 'solicitudesTotalesUltimosMeses', "nombreMeses", "nombreMesesSeleccionados", "faltaInformacion", "informacionErronea", "faltaMuestraFisica", "formatoImagenInadecuado", "medidaErronea", "descripcionDeProducto", "planoMalAcotado", "errorDeDigitacion", "errorTipoSustrato", "noViablePorRestricciones", "faltaCadParaCorte", "faltaOTChileexpress", "faltaOTLaboratorio"));
    }

    public function index()
    {
        // $ot
        return view('reports2.index');
    }
    ...
    public function reporte1()
    {
        ...
        $toDate = Carbon::now()->format('d/m/Y');
        return view('reports2.reporte1', compact('fromDate', 'toDate', "topOt", "topDesarrolloCompleto", "topOtrosDesarrollos"));
    }
    ...
    public function descargaReporteOT($fromDate, $toDate, $titulo, $ordenes = null)
    {
        ...
        foreach ($ots as $ot) {
            ...
            $ots_array[] = array(
                ...
                "ESTADO" => isset($ot->ultimoCambioEstado) ? $ot->ultimoCambioEstado->state->nombre : '',
    ...
    public function reportActiveOtsPerArea()
    {
        ...
        $otsDias = $otsDias->map(
            function ($ot) use (
                ...
                // $ot->dias_trabajados = round($ot->present()->diasTrabajados2($ot->tiempo_total), 1);
                $ot->dias_trabajados_venta = $ot->present()->diasTrabajados2PorArea($ot->tiempo_venta, 1);
                $ot->dias_trabajados_desarrollo = $ot->present()->diasTrabajados2PorArea($ot->tiempo_desarrollo, 2);
                $ot->dias_trabajados_muestra = $ot->present()->diasTrabajadosPorArea2SalaMuestra($ot->tiempo_muestra, 6);
                $ot->dias_trabajados_diseño = $ot->present()->diasTrabajados2PorArea($ot->tiempo_diseño, 3);
                $ot->dias_trabajados_catalogacion = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_catalogacion, 4), 1);
                $ot->dias_trabajados_precatalogacion = round($ot->present()->diasTrabajados2PorArea($ot->tiempo_precatalogacion, 5), 1);
        ...
        $count = 0;
        return view(
            'reports2.reportActiveOtsPerArea',
            compact(
                "area_actual",
                "users_actual",
                "areas",
    ...
    public function reportIndicadorSalaMuestra()
    {
        ...
        return view(
            'reports2.reportIndicadorSalaMuestra',
            compact(
                'mes',
                'years',
                'mesSeleccionado',
    ...
    public function reportDisenoEstructuralySalaMuestra()
    {
        ...
        */
        return view(
            'reports2.reportDisenoEstructuralySalaMuestra',
            compact(
                'mes',
                'years',
                'year',
    ...
    public function reportTiempoPrimeraMuestra()
    {
        ...
        for ($i = 1; $i <= $mes; $i++) {
            ...
            foreach ($ots_periodo_muestras_listas as $ot_muestra) {
                ...
                if ($fecha_termino_primera_muestra && $fromDateAux <= $fecha_termino_primera_muestra->ultimo_cambio_estado && $fecha_termino_primera_muestra->ultimo_cambio_estado <= $toDateAux) {
                    ...
                    $tiempo_desde_creación = get_working_hours_muestra2($fecha_creacion_ot->created_at, $fecha_termino_primera_muestra->ultimo_cambio_estado) / 24;
                    ...
                    $tiempo_desde_DE = get_working_hours_muestra2($fecha_ingreso_de_ot->created_at, $fecha_termino_primera_muestra->ultimo_cambio_estado) / 24;
        ...
        return view(
            'reports2.reportTiempoPrimeraMuestra',
            compact(
                'mes',
                'years',
                'mesSeleccionado',
    ...
    public function reportTiempoDisenadorExterno()
    {
        ...
        foreach ($ots_proveedor as $ot) {
            if (!in_array($ot->work_order_id, $array_ot_procesadas)) {
                ...
                if ($ingreso_diseño_grafico) {
                    ...
                    $tiempo_diseno_grafico = Management::select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))
                    ...
                    $tiempo_respuesta_diseño_grafico = round((($tiempo_diseno_grafico[0]->tiempo_total / 3600)) / 24, 1);
                }
                ...
                if ($ingreso_precatalogacion) {
                    ...
                    if ($ingreso_diseño_grafico) {
                        ...
                        //$tiempo_respuesta_diseño_grafico = round((get_working_hours2($ingreso_diseño_grafico->created_at, $ingreso_precatalogacion->created_at)/ 24),2);
                ...
                if ($ot->proveedor_id == 1) { //Proveedor Prinflex
                    ...
                    if ($ot->recibido_diseño_externo == 0) {
                        ...
                        $tiempo_respuesta_diseño_externo = get_working_hours2($ot->created_at, $fecha_final) / 24;
                        ...
                        $tiempo_respuesta_diseño_externo = get_working_hours2($ot->created_at, $recepcion_proveedor_prinflex->created_at) / 24;
                    ...
                    if ($ot->recibido_diseño_externo == 0) {
                        ...
                        //$tiempo_duracion_graphicbox += get_working_hours2($ot->created_at, $fecha_final)/ 24;
                        ...
                        $recepcion_proveedor_graphicbox = Management::where('proveedor_id', 2)
                            ...
                            ->where('gestion_id', $ot->id)
                            ->first();
                        $tiempo_respuesta_diseño_externo  = get_working_hours2($ot->created_at, $recepcion_proveedor_graphicbox->created_at) / 24;
                    ...
                    if ($ot->recibido_diseño_externo == 0) {
                        ...
                        //$tiempo_duracion_flexoclean += get_working_hours2($ot->created_at, $fecha_final)/ 24;
                        ...
                        $tiempo_respuesta_diseño_externo = get_working_hours2($ot->created_at, $recepcion_proveedor_flexoclean->created_at) / 24;
                    ...
                    if ($ot->recibido_diseño_externo == 0) {
                        ...
                        //$tiempo_duracion_artfactory += get_working_hours2($ot->created_at, $fecha_final)/ 24;
                        ...
                        $tiempo_respuesta_diseño_externo = get_working_hours2($ot->created_at, $recepcion_proveedor_artfactory->created_at) / 24;
        ...
        return view(
            'reports2.reportTiempoDisenadorExterno',
            compact(
                'mes',
                'years',
                'mesSeleccionado',
    ...
    public function query_ot_area_sala_muestra()
    {
        ...
        // DB::raw('MAX(work_orders.id) as max_work_order_id'));
    ...
    public function cantidad_ot_por_area_mes_actual($area_id, $vendedor_id)
    {
        ...
        $ots = WorkOrder::join('work_spaces', 'work_spaces.id', '=', 'work_orders.current_area_id')
            ->join('managements', 'work_orders.id', '=', 'managements.work_order_id')
            ->where('work_orders.current_area_id', '!=', 1)
            ->whereIn('creador_id', $vendedor_id)
            ->whereIn('current_area_id', $area_id)
            ->where('managements.management_type_id', 1)
            ->whereIn('managements.state_id', $estados_activos)
            ->where('managements.id', function ($q) {
                $q->select('id')
            ...
            ->select([
                'current_area_id',
                DB::raw('work_spaces.nombre as area_nombre'),
                DB::raw("(CASE
                WHEN work_spaces.nombre = 'Área de Ventas' THEN 'V'
                WHEN work_spaces.nombre = 'Área de Diseño Estructural' THEN 'DE'
                WHEN work_spaces.nombre = 'Área de Diseño Gráfico' THEN 'DG'
                ...
                WHEN work_spaces.nombre = 'Área de Precatalogación' THEN 'PC'
                WHEN work_spaces.nombre = 'Área de Muestras' THEN 'SM'
                ELSE 'Área' END) AS area_abreviatura_nombre"),
                DB::raw('COUNT(*) AS cantidad'),
                DB::raw('MAX(work_orders.id) AS last_id'), // <-- agregado
            ])
            ->groupBy('area_nombre', 'area_abreviatura_nombre', 'current_area_id')
            ->orderBy('last_id', 'desc') // <-- ordenar por agregado
            ->get();

        // mapa fijo para el gráfico
        $array_key_definition = ['DE' => 0, 'SM' => 0, 'DG' => 0, 'PC' => 0, 'C' => 0];
        foreach ($ots as $value) {
            $array_key_definition[$value->area_abreviatura_nombre] = (int)$value->cantidad;
        }

        return [
            'array_cantidad_ot_por_area' => array_values($array_key_definition),
            'array_keys_ot_por_area'     => array_keys($array_key_definition),
        ];
    }
    ...
    public function query_tiempos_area($fromDate, $toDate, $area_id, $vendedor_id)
    {
        ...
        $query = $query->withCount([
            'gestiones_report AS tiempo_venta' => function ($q) use ($fromDate, $toDate) {
                $q->select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))
                    ->where('management_type_id', 1)
        ...
        //     var_dump($value->id);
        //     var_dump($value->tiempo_venta);
        //     var_dump($value->tiempo_desarrollo);
        //     var_dump($value->tiempo_diseño);
        //     var_dump($value->tiempo_catalogacion);
        //     var_dump($value->tiempo_total);
        //     var_dump($value->tiempo_muestra);
        //    echo "<br>";
        // }
    ...
    //         //$ot->dias_trabajados = round($ot->present()->diasTrabajados2ReportDESM($ot->tiempo_total,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables), 1);
    //         //$ot->dias_trabajados_venta = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_venta, 1,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables), 1);
    //         $ot->dias_trabajados_desarrollo = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_desarrollo, 2, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //,$ultima_gestion_de_ot_de
    //         $ot->dias_trabajados_muestra = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_muestra, 6, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //,$ultima_gestion_de_ot_sm
    //         $ot->dias_trabajados_diseño = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_diseño, 3, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //,$ultima_gestion_de_ot_dg
    //         $ot->dias_trabajados_catalogacion = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_catalogacion, 4, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //, $ultima_gestion_de_ot_ct
    //         $ot->dias_trabajados_precatalogacion = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_precatalogacion, 5, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //, $ultima_gestion_de_ot_pc
    ...
    public function calcula_el_promedio($data, $fromDate, $toDate, $vendedorid, $promedio_anio)
    {
        $array_dias_trabajados_desarrollo = [];
        $array_dias_trabajados_muestra = [];
        $array_dias_trabajados_diseno = [];
        $array_dias_trabajados_catalogacion = [];
        $array_dias_trabajados_precatalogacion = [];
        ...
        // ✅ Configuración para 24/7 (sin feriados ni horario limitado)
        $dic_variables = [
            'ini_time_lun_jue' => [0, 0],
            'end_time_lun_jue' => [23, 59],
            'ini_time_viernes' => [0, 0],
            'end_time_viernes' => [23, 59],
            'skipdates' => [], // sin feriados
            'current_date' => Carbon::now()
        ];

        foreach ($data as $ot) {
            $ot->dias_trabajados_desarrollo = round($ot->present()->diasTrabajados2PorAreaReportDESM(
                $ot->tiempo_desarrollo,
                2,
                $dic_ultimoCambioEstado_byWorkOrderId,
                $dic_variables,
                $fromDate,
                $toDate,
                $promedio_anio
            ...
            $ot->dias_trabajados_muestra = round($ot->present()->diasTrabajados2PorAreaReportDESM(
                $ot->tiempo_muestra,
                6,
                $dic_ultimoCambioEstado_byWorkOrderId,
                $dic_variables,
            ...
            $ot->dias_trabajados_diseño = round($ot->present()->diasTrabajados2PorAreaReportDESM(
                $ot->tiempo_diseño,
                3,
                $dic_ultimoCambioEstado_byWorkOrderId,
                $dic_variables,
            ...
            $ot->dias_trabajados_catalogacion = round($ot->present()->diasTrabajados2PorAreaReportDESM(
                $ot->tiempo_catalogacion,
                4,
                $dic_ultimoCambioEstado_byWorkOrderId,
                $dic_variables,
                $fromDate,
            ...
            $ot->dias_trabajados_precatalogacion = round($ot->present()->diasTrabajados2PorAreaReportDESM(
                $ot->tiempo_precatalogacion,
                5,
                $dic_ultimoCambioEstado_byWorkOrderId,
                $dic_variables,
                $fromDate,
            ...
            $array_dias_trabajados_desarrollo[] = $ot->dias_trabajados_desarrollo;
            $array_dias_trabajados_muestra[] = $ot->dias_trabajados_muestra;
            $array_dias_trabajados_diseno[] = $ot->dias_trabajados_diseño;
            $array_dias_trabajados_catalogacion[] = $ot->dias_trabajados_catalogacion;
            $array_dias_trabajados_precatalogacion[] = $ot->dias_trabajados_precatalogacion;
        }
    ...
    //     //Horario Lunes a Jueves
    //     $horarioSalaMuestra = SystemVariable::where('name', 'SinHorarioSalaMuestras')
    //         ->where('deleted', 0)
    //         ->first();
    ...
    //         //$ot->dias_trabajados_venta = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_venta, 1,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables), 1);
    //         $ot->dias_trabajados_desarrollo = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_desarrollo, 2, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //,$ultima_gestion_de_ot_de
    //         $ot->dias_trabajados_muestra = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_muestra, 6, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables_muestra, $fromDate, $toDate, $promedio_anio), 1); //,$ultima_gestion_de_ot_sm
    //         $ot->dias_trabajados_diseño = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_diseño, 3, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //,$ultima_gestion_de_ot_dg
    //         $ot->dias_trabajados_catalogacion = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_catalogacion, 4, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //, $ultima_gestion_de_ot_ct
    //         $ot->dias_trabajados_precatalogacion = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_precatalogacion, 5, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, $promedio_anio), 1); //, $ultima_gestion_de_ot_pc
    ...
    public function calcula_el_promedio_mes_actual($data, $fromDate, $toDate, $vendedorid, $promedio_anio)
    {
        ...
        // ✅ CONFIGURACIÓN 24/7 para TODAS las áreas
        $dic_variables = [
            'ini_time_lun_jue' => [0, 0],
            'end_time_lun_jue' => [23, 59],
            'ini_time_viernes' => [0, 0],
        ...
        // ✅ CONFIGURACIÓN 24/7 para SALA DE MUESTRAS
        $dic_variables_muestra = [
            'ini_time_lun_jue' => [0, 0],
            'end_time_lun_jue' => [23, 59],
            'ini_time_viernes' => [0, 0],
        ...
        // Conteo de elementos con días válidos
        $count_dias_trabajados_desarrollo = self::conteo_de_elementos($array_dias_trabajados_desarrollo, 0);
        $count_dias_trabajados_muestra = self::conteo_de_elementos($array_dias_trabajados_muestra, 0);
        $count_dias_trabajados_diseno = self::conteo_de_elementos($array_dias_trabajados_diseno, 0);
        $count_dias_trabajados_catalogacion = self::conteo_de_elementos($array_dias_trabajados_catalogacion, 0);
        ...
        // Agregar tiempos actuales por área
        $areas = [2, 3, 6, 4, 5]; // desarrollo, diseño, muestra, catalogación, precatalogación
        foreach ($areas as $area) {
            $resp = self::tiempo_ot_area_actual($fromDate, $toDate, $area, $ot_ids);
            switch ($area) {
                case 2:
                    $suma_dias_trabajados_desarrollo += $resp['tiempo'];
                    $count_dias_trabajados_desarrollo += $resp['cantidad'];
                    break;
                case 3:
                    $suma_dias_trabajados_diseno += $resp['tiempo'];
                    $count_dias_trabajados_diseno += $resp['cantidad'];
                    break;
                case 6:
                    $suma_dias_trabajados_muestra += $resp['tiempo'];
                    $count_dias_trabajados_muestra += $resp['cantidad'];
                    break;
                case 4:
                    $suma_dias_trabajados_catalogacion += $resp['tiempo'];
                    $count_dias_trabajados_catalogacion += $resp['cantidad'];
                    break;
                case 5:
                    $suma_dias_trabajados_precatalogacion += $resp['tiempo'];
                    $count_dias_trabajados_precatalogacion += $resp['cantidad'];
                    break;
            }
        }
        ...
        return [
            ...
            // Extras para depuración o cálculos adicionales
            'count_dias_trabajados_desarrollo' => $count_dias_trabajados_desarrollo,
            'count_dias_trabajados_muestra' => $count_dias_trabajados_muestra,
            'count_dias_trabajados_diseno' => $count_dias_trabajados_diseno,
            'count_dias_trabajados_catalogacion' => $count_dias_trabajados_catalogacion,
    ...
    //         //$ot->dias_trabajados = round($ot->present()->diasTrabajados2ReportDESM($ot->tiempo_total,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables), 1);
    //         //$ot->dias_trabajados_venta = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_venta, 1,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables), 1);
    //         $ot->dias_trabajados = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_duracion, $area, $dic_ultimoCambioEstado_byWorkOrderId, $dic_variables, $fromDate, $toDate, 2), 1); //,$ultima_gestion_de_ot_de
    //         //$ot->dias_trabajados_muestra = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_muestra, 6,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables,$fromDate, $toDate,$promedio_anio), 1);//,$ultima_gestion_de_ot_sm
    //         //$ot->dias_trabajados_diseño = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_diseño, 3,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables,$fromDate, $toDate,$promedio_anio), 1);//,$ultima_gestion_de_ot_dg
    //         //$ot->dias_trabajados_catalogacion = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_catalogacion, 4,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables,$fromDate, $toDate,$promedio_anio), 1);//, $ultima_gestion_de_ot_ct
    //         //$ot->dias_trabajados_precatalogacion = round($ot->present()->diasTrabajados2PorAreaReportDESM($ot->tiempo_precatalogacion, 5,$dic_ultimoCambioEstado_byWorkOrderId, $dic_variables,$fromDate, $toDate,$promedio_anio), 1);//, $ultima_gestion_de_ot_pc
    ...
    public function calcula_el_promedio_anio_actual_aux($data, $fromDate, $toDate, $vendedorid, $area)
    {
        ...
        // ✅ Simulación de horario 24/7
        $dic_variables = [];
        $dic_variables['ini_time_lun_jue'] = [0, 0];
        $dic_variables['end_time_lun_jue'] = [23, 59];
        $dic_variables['ini_time_viernes'] = [0, 0];
        $dic_variables['end_time_viernes'] = [23, 59];
        $dic_variables['skipdates'] = []; // sin feriados
        $dic_variables['current_date'] = Carbon::now();

        foreach ($data as $ot) {
            // Se calcula el tiempo en días trabajados basado en duración real (tiempo_duracion)
            $ot->dias_trabajados = round(
                $ot->present()->diasTrabajados2PorAreaReportDESM(
                    $ot->tiempo_duracion,
                    $area,
                    $dic_ultimoCambioEstado_byWorkOrderId,
                    $dic_variables,
            ...
            $array_dias_trabajados[] = $ot->dias_trabajados;
        }

        // Conteo de OT válidas (que tengan días mayores a 0)
        $count_ot_trabajados = self::conteo_de_elementos($array_dias_trabajados, 0);

        // Suma total de días trabajados
        $suma_ot_trabajados = array_sum($array_dias_trabajados);
            ...
            foreach ($gestiones_ot as $gestion_ot) {
                ...
                if ($gestion_ot_anterior && $gestion_ot_anterior->mostrar != 0) {
                    ...
                    if ($diff_days > 30) {
                        ...
                        if ($area == 6) {
                            $diff_mes_inicio    = get_working_hours_muestra2($gestion_ot_anterior->created_at, $fecha_gestion_anterior_fin) * 3600;
                        } else {
                            $diff_mes_inicio    = get_working_hours2($gestion_ot_anterior->created_at, $fecha_gestion_anterior_fin) * 3600;
                        }
                        ...
                        $gestion_anterior_insert->duracion_segundos_24h     = $diff_mes_inicio;
                        ...
                        for ($i = 30; $diff_days > $i; $i += 30) {
                            ...
                            if ($area == 6) {
                                $diff_mes_aux    = get_working_hours_muestra2($fecha_aux_ini, $fecha_aux_fin) * 3600;
                            } else {
                                $diff_mes_aux    = get_working_hours2($fecha_aux_ini, $fecha_aux_fin) * 3600;
                            }
                            ...
                            $gestion_anterior_insert->work_order_id         = $gestion_ot->work_order_id;
                            $gestion_anterior_insert->work_space_id         = $area;
                            $gestion_anterior_insert->duracion_segundos_24h     = $diff_mes_aux;
                        ...
                        if ($area == 6) {
                            $diff_mes_actual    = get_working_hours_muestra2($fecha_gestion_actual_ini, $gestion_ot->created_at) * 3600;
                        } else {
                            $diff_mes_actual    = get_working_hours2($fecha_gestion_actual_ini, $gestion_ot->created_at) * 3600;
                        }

                        $gestion_ot_actual_update   = Management::where('id', $gestion_ot->id)->update(['duracion_segundos_24h' => $diff_mes_actual, 'duracion_segundos' => $diff_mes_actual]);
                    } else {
                        ...
                        if ($mes_gestion_actual != $mes_gestion_anterior) {
                            ...
                            if ($area == 6) {
                                $diff_mes_actual    = get_working_hours_muestra2($fecha_gestion_actual_ini, $gestion_ot->created_at) * 3600;
                                $diff_mes_anterior  = get_working_hours_muestra2($gestion_ot_anterior->created_at, $fecha_gestion_anterior_fin) * 3600;
                            } else {
                                $diff_mes_actual    = get_working_hours2($fecha_gestion_actual_ini, $gestion_ot->created_at) * 3600;
                                $diff_mes_anterior  = get_working_hours2($gestion_ot_anterior->created_at, $fecha_gestion_anterior_fin) * 3600;
                            }


                            $gestion_ot_actual_update = Management::where('id', $gestion_ot->id)->update(['duracion_segundos_24h' => $diff_mes_actual]);
                            ...
                            $gestion_anterior_insert->work_order_id         = $gestion_ot->work_order_id;
                            $gestion_anterior_insert->work_space_id         = $area;
                            $gestion_anterior_insert->duracion_segundos_24h     = $diff_mes_anterior;
            ...
            if ($gestion_ot &&  !in_array($gestion_ot->state_id, [8, 9, 11, 13]) && $gestion_ot->mostrar != 0) {
                ...
                if ($mes_ultima_gestion != $mes_actual) {
                    ...
                    if ($diff_days > 30) {
                        ...
                        $fecha_gestion_anterior_ini = Carbon::createFromFormat('Y-m-d', $anio_gestion_anterior . '-' . $mes_gestion_anterior . '-' . '1')->startOfMonth();
                        if ($area == 6) {
                            $diff_mes_inicio    = get_working_hours_muestra2($gestion_ot->created_at, $fecha_gestion_anterior_fin) * 3600;
                        } else {
                            $diff_mes_inicio    = get_working_hours2($gestion_ot->created_at, $fecha_gestion_anterior_fin) * 3600;
                        }
                        ...
                        if ($mes_actual != $mes_gestion_anterior) {
                            ...
                            */
                            ...
                            // $diff_mes_actual    = get_working_hours2($fecha_gestion_actual_ini, $gestion_ot->created_at) * 3600;
                            if ($area == 6) {
                                $diff_mes_anterior  = get_working_hours_muestra2($gestion_ot->created_at, $fecha_gestion_anterior_fin) * 3600;
                            } else {
                                $diff_mes_anterior  = get_working_hours2($gestion_ot->created_at, $fecha_gestion_anterior_fin) * 3600;
                            }
                ...
                if ($gestion_ot &&  !in_array($gestion_ot->state_id, [8, 9, 11, 13]) && $gestion_ot->mostrar == 0) {
                    ...
                    if ($mes_ultima_gestion != $mes_actual) {
                        ...
                        if ($diff_days >= $diff_days_compare) {
                            ...
                            if ($area == 6) {
                                $diff_mes_inicio    = get_working_hours_muestra2($fecha_ini_aux, $fecha_fin_aux) * 3600;
                            } else {
                                $diff_mes_inicio    = get_working_hours2($fecha_ini_aux, $fecha_fin_aux) * 3600;
                            }
    ...
    public function tiempo_ot_area_actual($fromDate, $toDate, $area, $ot_ids)
    {
        ...
        $lastDate = $isCurrentMonth ? Carbon::now() : $to;

        // 1) IDs de OTs vigentes del área (excluyendo $ot_ids)
        $otIds = WorkOrder::where('current_area_id', $area)
            ->whereNotIn('id', $ot_ids)
            ->where('active', 1)
            ->pluck('id');

        if ($otIds->isEmpty()) {
            return ['tiempo' => 0.0, 'cantidad' => 0];
        }

        // 2) Última gestión (management_type_id=1) anterior a $from por OT
        $gestiones = Management::select('work_order_id', 'state_id', 'created_at')
            ->whereIn('work_order_id', $otIds)
            ->where('management_type_id', 1)
            ->where('created_at', '<', $from)
            ->orderBy('work_order_id')
            ->orderByDesc('created_at')
            ->get()
            ->unique('work_order_id'); // nos quedamos con la última por OT

        $estadosInactivos = [8, 9, 11, 13];

        $horasTotales = 0.0;
        $cantidad = 0;

        foreach ($gestiones as $g) {
            if (in_array($g->state_id, $estadosInactivos, true)) {
                continue;
            }

            $ultima = $g->created_at instanceof Carbon ? $g->created_at : Carbon::parse($g->created_at);

            // si la última gestión NO es del mismo mes de la consulta, contamos desde inicio de ese mes
            if (!$ultima->isSameMonth($from)) {
                $inicioMes = $from->copy()->startOfMonth();

                $diffSeg = max(0, $lastDate->getTimestamp() - $inicioMes->getTimestamp());
                $horasTotales += $diffSeg / 3600; // horas reales 24/7
                $cantidad++;
            }
        }
    ...
    //                 if ($area == 6) {
    //                     $diff_mes_actual    = get_working_hours_muestra2($fecha_consulta_actual_ini, $last_date) * 3600;
    //                 } else {
    //                     $diff_mes_actual    = get_working_hours2($fecha_consulta_actual_ini, $last_date) * 3600;
    //                 }
    ...
    //     //dd((($tiempo_total / 3600) / 24),$i,round((($tiempo_total / 3600) / 24)/$i));
    //     $response['tiempo'] = ($tiempo_total / 3600) / 24;
    //     $response['cantidad'] = $i;
    ...
    public function datos_disenador_estructural($mes, $year)
    {
        ...
        if ($disenador_estructural->count() > 0) {
            $disenador_estructural = $disenador_estructural->map(function ($disenador) use ($mes, $year) {
                ...
                $ots_gestion = Management::select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))
                    ->where('managements.management_type_id', 1)
                ...
                $ots_gestion_aux = Management::select("managements.work_order_id")
                    ...
                    ->get();
                //Se obtienen las gestiones donde participa el diseñador del antes del periodo
                $ots_gestion_aux_tiempo = Management::select(DB::raw("SUM(duracion_segundos_24h) as tiempo_total"))
                    ->where('managements.management_type_id', 1)
                    ->where('managements.work_space_id', 2)
                    ->where('managements.user_id', $disenador->id)
                    ->whereNotIn('managements.work_order_id', $ots_in)
                ...
                $disenador->tiempo_total = ($disenador->ots->sum('tiempo_total') + $ots_gestion->sum('tiempo_total') + $ots_gestion_aux_tiempo->sum('tiempo_total')) / 3600 / 24;
        ...
        foreach ($ots_proveedor_periodo as $ot_proveedor) {
            if (!in_array($ot_proveedor->work_order_id, $array_ot_procesadas)) {
                ...
                if ($ot_proveedor->recibido_diseño_externo != 0) {
                    ...
                    $tiempo_respuesta_diseño_externo = round((get_working_hours2($ot_proveedor->created_at, $recepcion_proveedor_prinflex->created_at) / 24), 2);
