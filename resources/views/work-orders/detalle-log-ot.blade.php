...
<h1 class="page-title">Lista de Log</h1>

<form id="filtros" class="py-3" action="{{ route('detalleLogOt', $ot_id) }}" method="get" enctype="multipart/form-data">
	@csrf
	<div class="form-row ">
		<div class="col-1">
...
<div class="container-table mt-3 bg-white border">
	<table class="table table-status table-hover" style="width:100%;">
		<thead>
			<tr>
				<th style="width:10%;">OT</th>
				<th style="width:10%;">ID Cambio</th>
			...
			@if($dato == null)
				@foreach($bitacora_ot as $bitacora)
					@foreach($bitacora->datos_modificados as $key => $value)
						<tr>
							<td>{{$bitacora->work_order_id}}</td>
							<td>{{$bitacora->id}}</td>
							<td>{{$bitacora->created_at}}</td>
							<td>{{$bitacora->observacion}}</td>
							<td>{{$value['texto']}}</td>
							@if($bitacora->operacion=='Modificación')
								<td>{{$value['antiguo_valor']['descripcion'] === '' || $value['antiguo_valor']['descripcion'] === null ? 'Campo Vacío' : $value['antiguo_valor']['descripcion'] }}</td>
								<td>{{$value['nuevo_valor']['descripcion'] === '' || $value['nuevo_valor']['descripcion'] === null ? 'Campo Vacío' : $value['nuevo_valor']['descripcion'] }}</td>
							@else
								<td>N/A</td>
					...
					@foreach($bitacora->datos_modificados as $key => $value)
						@if(in_array($value['texto'], $dato))
							<tr>
								...
								<td>{{$bitacora->observacion}}</td>
								<td>{{$value['texto']}}</td>
								@if($bitacora->operacion=='Insercion')
									<td>0</td>
									<td>{{$value['valor'][1] === '' || $value['valor'][1] === null ? 'Campo Vacío' : $value['valor'][1] }}</td>
								@else
									<td>{{$value['antiguo_valor']['descripcion'] === '' || $value['antiguo_valor']['descripcion'] === null ? 'Campo Vacío' : $value['antiguo_valor']['descripcion'] }}</td>
									<td>{{$value['nuevo_valor']['descripcion'] === '' || $value['nuevo_valor']['descripcion'] === null ? 'Campo Vacío' : $value['nuevo_valor']['descripcion'] }}</td>
								@endif
