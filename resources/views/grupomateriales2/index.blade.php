...
@section('content')
<!-- Titulo -->
<h1 class="page-title">Mantenedor de Grupos Materiales 2
  <a href="{{ route('mantenedores.grupo-materiales-2.create') }}" class="btn btn-primary rounded-pill ml-3 px-5">Crear</a>
</h1>

<!-- Filtros -->
<form id="filtros" class="py-3" action="{{ route('mantenedores.grupo-materiales-2.list') }}" method="get" enctype="multipart/form-data">
  @csrf
  <div class="form-row">
    <div class="col-sm-12 col-md-3 col-lg-3">
      <div class="form-group">
        ...
        <select name="id[]" id="id" class="form-control form-control-sm" multiple data-live-search="true" title="Seleccionar..." data-selected-text-format="count > 1">
          {!! optionsSelectObjetfilterMultiple($grupomateriales_filter,'id',['codigo'],'') !!}
        </select>
      </div>
    </div>
    <div class="col-sm-12 col-md-3 col-lg-3">
      <div class="form-group">
        ...
        <select name="pruduct_type_id[]" id="pruduct_type_id" class="form-control form-control-sm" multiple data-live-search="true" title="Seleccionar..." data-selected-text-format="count > 1">
          {!! optionsSelectObjetfilterMultiple($producttype_filter,'id',['codigo','descripcion'],'') !!}
        </select>
      </div>
    </div>
    <div class="col-2">
  ...
  <table class="table table-status table-hover actions states">
    <thead>
      <tr>
        ...
        <th> {!! order_column('Tipo Producto','product_type_id','ASC') !!} </th>
        <th>Estado</th>
        <th>Acciones</th>
      </tr>
    </thead>
    <tbody>
      ...
      <tr class="{{ $grupomaterial->active == 1 ? '' : 'text-muted' }}">
        ...
        <td>{{ $grupomaterial->tipo_producto->codigo .' '.$grupomaterial->tipo_producto->descripcion}}</td>
        <td>
        ...
        <td>
          <a href="{{route('mantenedores.grupo-materiales-2.edit', $grupomaterial->id)}}">
            <div class="material-icons md-14" data-toggle="tooltip" title="Editar">edit</div>
          </a>
          @if($grupomaterial->active == 0 )
          <form method="POST" action="{{ route('mantenedores.grupo-materiales-2.active', $grupomaterial->id) }}" style="display: inline;">
            @method('put')
            @csrf
          ...
          @else
          <form method="POST" id="form_{{ $grupomaterial->id }}" action="{{ route('mantenedores.grupo-materiales-2.inactive', $grupomaterial->id) }}" style="display: inline;">
            @method('put')
            @csrf
