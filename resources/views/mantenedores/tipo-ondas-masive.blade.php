...
<div class="row">
  <div class="col-12">
    <div class="page-title" style="display:inline">Carga masiva de Ondas

    </div>
    <!-- <a class="btn btn-success btn-sm" data-attribute="link" href="/files/Carga Masiva Corrugados.xlsx" download title="Descargar">Archivo Ejemplo Ondas</a> -->
    <a class="btn btn-info flot-right" href="{{ route('mantenedores.cotizador.ondas.descargar_excel_ondas') }}" title="Descargar Listado Ondas">Descargar Listado Ondas
      <div class="material-icons md-14" data-toggle="tooltip" title="Descargar Listado Ondas" style="color:white;">download</div> </a>
    </a>
  </div>
</div>
<br>
<div class="normalForm">
  <!-- formulario: -->
  <form id="form-carga-ondas" method="POST" enctype="multipart/form-data" action="{{ route('mantenedores.cotizador.ondas.uploading') }}">
    @csrf
    <div class="col-xs-12 col-md-12">
      <div class="row">
        <div id="container-boton-carga" class="col-2 row text-center">
          ...
          <div class="col-12">
            <button id="btn-cargar-ondas" disabled type="submit" class="btn btn-success ">Procesar Papeles</button>
          </div>
...
<div class="row ">
  <div class="col">
    <div class="card-counter primary">
      ...
      <span class="count-name">Total Ondas</span>
    </div>
  </div>
  <div class="col">
    <div class="card-counter warning">
      ...
      <span class="count-name">Ondas Actualizados</span>
    </div>
  </div>
  <div class="col">
  ...
  <div class="col-12 ">
    <div class="" style="margin: 5px;;background-color: #f2f4f5;height: 40px;border-radius: 5px;">
      <ul class="legend" style="display: flex;justify-content:space-around">
        <li data-toggle="tooltip" data-html="true" title="Total Ondas"><span style="background-color:#4491e4"></span> Total Ondas</li>
        <li data-toggle="tooltip" data-html="true" title="Ondas Actualizados"><span style="background-color:#faffa8"></span> Ondas Actualizados</li>
  ...
  <table id="listadoOndasMasivos" class="table table-status table-hover text-center">
    ...
    <tbody>
      ...
      <tr id="onda-row-{{ $onda->id }}" class="{{ $onda->active == 1 ? '' : 'text-muted' }}">
        <td>{{ $onda->id }}</td>
        <td>{{ $onda->onda }}</td>
        <td>{{ $onda->espesor_promedio }}</td>
        <td>{{ $onda->espesor_maximo }}</td>
        <td>{{ $onda->espesor_minimo }}</td>
      </tr>
      @endforeach
    </tbody>
  </table>
...
<script src="{{ asset('js/mantenedores/tipo-onda-masivo.js') }}"></script>
@endsection