...
<div class="row">
  <div class="col-12">
    <div class="page-title" style="display:inline">Carga masiva de Cartones Esquineros

    </div>
    <!-- <a class="btn btn-success btn-sm" data-attribute="link" href="/files/Carga Masiva Esquineros.xlsx" download title="Descargar">Archivo Ejemplo Cartones</a> -->
    <a class="btn btn-info flot-right" href="{{ route('mantenedores.cotizador.cartones-esquineros.descargar_excel_cartones_esquineros') }}" title="Descargar Listado Cartones Esquineros">Descargar Listado Cartones Esquineros
...
<div class="normalForm">
  <!-- formulario: -->
  <form id="form-carga-cartones" method="POST" enctype="multipart/form-data" action="{{ route('mantenedores.cotizador.cartones-esquineros.uploading') }}">
    @csrf
  ...
  <table id="listadoCartonesMasivos" class="table table-status table-hover text-center">
    <thead>
      <tr>
        <th width="50px">ID</th>
        <th>Código</th>
        <th>Código Papel 1</th>
        <th>Código Papel 2</th>
        <th>Código Papel 3</th>
        <th>Código Papel 4</th>
        <th>Código Papel 5</th>
        <th>Resistencia</th>
        <th>Ancho Esquinero</th>
        <!-- <th>Onda</th>
        <th>Peso Bruto (g)</th>
        <th>Espesor (mm)</th>
        <th>Color</th>
    ...
    <tbody>
      ...
      <tr id="carton-row-{{ $carton->orden }}" class="{{ $carton->active == 1 ? '' : 'text-muted' }}">
        <td>{{ $carton->id }}</td>
        <td>{{ $carton->codigo }}</td>
        <td>{{$carton->codigo_papel_1 != '0' && $carton->codigo_papel_1 != "" ? $carton->codigo_papel_1 : ""}}</td>
        <td>{{$carton->codigo_papel_2 != '0' && $carton->codigo_papel_2 != "" ? $carton->codigo_papel_2 : ""}}</td>
        <td>{{$carton->codigo_papel_3 != '0' && $carton->codigo_papel_3 != "" ? $carton->codigo_papel_3 : ""}}</td>
        <td>{{$carton->codigo_papel_4 != '0' && $carton->codigo_papel_4 != "" ? $carton->codigo_papel_4 : ""}}</td>
        <td>{{$carton->codigo_papel_5 != '0' && $carton->codigo_papel_5 != "" ? $carton->codigo_papel_5 : ""}}</td>
...
<script src="{{ asset('js/mantenedores/carton-esquinero-masivo.js') }}"></script>
@endsection