...
<div class="row">
	<div class="col-9">
		...
		<!-- if((isset(auth()->user()->role->area) && (!auth()->user()->isVendedor() && !auth()->user()->isJefeVenta()) && $usuarioAsignado && $ot->current_area_id == auth()->user()->role->area->id) || ((auth()->user()->isVendedor() || auth()->user()->isJefeVenta())) || (isset(auth()->user()->role->area) && auth()->user()->role->area->id == 4 && $usuarioAsignado && ($ot->current_area_id == 4 || $ot->current_area_id == 5)) || ) -->
		@if((isset(auth()->user()->role->area) && $usuarioAsignado) || (Auth()->user()->isJefeMuestras() || Auth()->user()->isJefeDesarrollo()))
		...
		@if((isset(auth()->user()->role->area) && $usuarioAsignado) && (Auth()->user()->isIngeniero() || auth()->user()->isJefeVenta()||auth()->user()->isVendedor() || auth()->user()->isTecnicoMuestras()) || (Auth()->user()->isJefeMuestras() || Auth()->user()->isJefeDesarrollo()))
		@include('partials/muestras-ot',['ot' => $ot,'cartons' => $cartons,'cartons_muestra' => $cartons_muestra,'cads' => $cads,'comunas' => $comunas])
		@endif


		<!-- Si esta anulada o perdida permite recuperarla  -->
		@elseif(!empty($ot->ultimoCambioEstado) && ($ot->ultimoCambioEstado->state_id == 9 || $ot->ultimoCambioEstado->state_id == 11) && auth()->user()->role->area->id == 1)
	...
	<div class="modal-dialog modal-md " style="width:100%">
		<div class="modal-content">
			...
			<div class="modal-body">
				<form id="form-codigo-material" method="GET" action="{{ route('duplicateOt',$ot->id) }}" class="form-row form-codigo-material">
					@csrf
...
@endsection
@section('myjsfile')
<script src="{{ asset('js/ot-old-vertion/ot-managements.js') }}"></script>
<script>
...
<script src="{{ asset('js/ot-old-vertion/modalCodigoMaterial.js') }}"></script>
<script src="{{ asset('js/ot-old-vertion/modalCadMaterial.js') }}"></script>
