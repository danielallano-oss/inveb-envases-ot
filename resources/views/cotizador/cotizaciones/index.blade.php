...
<h1 class="page-title">Cotizaciones
  ...
  <a href="{{ route('cotizador.crear_cotizacion') }}" class="btn btn-primary rounded-pill ml-3 px-5">Crear Cotización</a>
  <!-- <a href="{{ route('cotizador.crear_areahc') }}" class="btn btn-primary rounded-pill ml-3 px-5">Cálculo HC</a> -->
  @endif
</h1>
  ...
  <div class="form-row">
    ...
    <div class="col-2">
      <div class="form-group">
        ...
        <select name="creador_id[]" id="creador_id" class="form-control form-control-sm" multiple data-live-search="true" title="Seleccionar..." data-selected-text-format="count > 1">
          {!! optionsSelectObjetfilterMultiple($creadores,'creador_id',['rut','nombre','apellido'],'') !!}
        </select>
      </div>
    </div>
    @endif
    ...
    <div class="col-2">
      <div class="form-group">
        <label>OT</label>
        <select name="work_order_id[]" id="work_order_id" class="form-control form-control-sm" multiple data-live-search="true" title="Selecciona..." data-selected-text-format="count > 1">
          {!! optionsSelectObjetfilterMultiple($ots,'work_order_id',['id'],' ') !!}
        </select>
      </div>
    </div>
    <div class="col-2">
  ...
  <table class="table table-status table-hover text-center">
    ...
    <tbody>
      @foreach($cotizaciones as $cotizacion)
        ...
        @if(auth()->user()->id != $cotizacion->user_id && !Auth()->user()->isJefeCotizador())
          @if($cotizacion->estado_id == 2)
            <tr class="{{ $cotizacion->active == 1 ? '' : 'text-muted' }}" style="background-color: #cdfaf2;">
              <td>{{ $cotizacion->id }}</td>
              ...
              <td>
                ...
                @if(Auth()->user()->isJefeCotizador() && auth()->user()->id != $cotizacion->user_id || (Auth()->user()->isAdmin()))
                ...
                @else
                @if(auth()->user()->id != $cotizacion->user_id)
                  @if($cotizacion->estado_id == 2)
                    <a href="{{route('cotizador.aprobar_cotizacion_externo', $cotizacion->id)}}">
                      <div class="material-icons md-14" data-toggle="tooltip" title="Aprobar/Rechazar">check_circle</div>
                    </a>
                  @endif
                @else
                  <a href="{{route('cotizador.editar_cotizacion', $cotizacion->id)}}">
                ...
                @if( (auth()->user()->id == $cotizacion->user_id || (Auth()->user()->isJefeCotizador() && auth()->user()->id != $cotizacion->user_id)) && $cotizacion->active == 1 && $cotizacion->estado_id == 3)
                ...
                <a style="" class="" href="{{ route('cotizador.detalle_costos',['id'=>$cotizacion->id]) }}">
                  <div class="material-icons md-14 ml-1" data-toggle="tooltip" title="Descargar Detalle Costos" style="">description</div>
                </a>
                @endif
              </td>
            </tr>
