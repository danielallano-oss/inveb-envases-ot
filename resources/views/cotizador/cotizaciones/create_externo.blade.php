...
<div class="row">
  ...
  @if($cotizacion)
  <div class="col-2">
    <a id="duplicarCotizacion" href="{{ route('cotizador.duplicarCotizacion', $cotizacion->id) }}" class="btn btn-block btn-success">Duplicar Cotización</a>
  </div>
  @endif
...
<div class="row">
  ...
  <div class="col-2">
    <a id="duplicarCotizacion" href="{{ route('cotizador.duplicarCotizacion', $cotizacion->id) }}" class="btn btn-success btn-block ">Duplicar Cotización</a>
  </div>
  @if($cotizacion && $cotizacion->estado_id >= 3  && auth()->user()->id == $cotizacion->user_id && $cotizacion->active == 1)
  <div class="col-2">
      <a href="{{ route('cotizador.versionarCotizacion', $cotizacion->id) }}" class="btn btn-success btn-block" onclick="event.preventDefault();
...
<div class="row mb-3">
  <div class="col-12 p-2">
    <form id="formCotizacion" method="POST" action="{{ route('cotizador.crear_areahc') }}">
      @csrf
      @include('cotizador.cotizaciones.form_externo', ['tipo' => "create",'cotizacion' => $cotizacion,'class' => '',])
    </form>
    <!-- Modulo de aprobacion  -->
    @include('partials/aprobacion-cotizaciones', ['cotizacion'=>$cotizacion])
...
<form id="retomarCotizacionForm" action="{{ route('cotizador.retomarCotizacionExterno', $cotizacion->id) }}" method="POST" style="display: none;">
  @csrf
</form>
@endif
@endsection
...
<script src="{{ asset('js/cotizador/detalle-cotizacion-externo.js?'.date('ymdhis'))}}"></script>
<script src="{{ asset('js/cotizador/cotizacion-validation.js') }}"></script>
<script src="{{ asset('js/cotizador/cotizacion_externo.js') }}"></script>
