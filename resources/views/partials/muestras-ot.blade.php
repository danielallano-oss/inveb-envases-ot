...
<section id="muestras" class="py-4" data-container="muestras">
    ...
    font-size: 24px;">Muestras
                        ...
                        @if (auth()->user()->role_id == 6 && ($ot->tipo_solicitud == 6 && $ot->ajuste_area_desarrollo == 1))
                            <a href="#" id="crear_muestra_masiva" class="btn btn-primary rounded-pill ml-3 px-5" data-toggle="modal" data-target="#modalMuestrasExcel">Crear Muestras Masivas</a>
					...
					<table class="table table-status table-hover actions states">
						...
						<tbody>
							...
							<tr>
								@endif
								<td width="50px">{{ $muestra->muestra_excel == 1 ? $muestra->ot_id_excel : $muestra->id }}</td>
								...
								@endif
                                @if($muestra->destinatarios_id[0] == 0)
                                    <td>--</td>
                                @endif
								<td>{{[2=>"Retira Diseñador VB",1=>"Retira Ventas VB",4=>"Envío Cliente VB",5=>"Retira Diseñador Revisión",3=>"Envío Laboratorio",0 =>"Sin Destinatario"][$muestra->destinatarios_id[0]]}}</td>
                                ...
                                @elseif ($muestra->destinatarios_id[0] == 0)
                                    <td>--</td>
                                    <td>--</td>
								@endif
											...
											<div id="acciones_{{$muestra->id}}" class="form-group">
												...
												@else
													...
													@endif
														...
														@if((isset(auth()->user()->role->area) && $usuarioAsignado) && (Auth()->user()->isIngeniero() && Auth()->user()->isVendedor() ) )
															&nbsp;&nbsp;
															<a id="eliminar-muestra" data-toggle="modal" data-target="#modal-eliminar-muestra" href="#" class=" modalVerDetalle" data-id="{{$muestra->id}}" data-id-excel="{{$muestra->ot_id_excel}}" >
																<div class="material-icons md-14" data-toggle="tooltip" title="Eliminar">delete</div>
															</a>
														@endif
							...
							<div class="col-12 mb-2">
								<div class="card">
									<div class="card-body inputs_muestra">
										...
										<div class="row ">
											<div class="col-4">
                                                <div class="cad_text" id="cad_input_container_muestra" style="display:none">
													{!! armarInputCreateEditOT('cad', 'CAD:', 'text',$errors, $ot, 'form-control', '', '') !!}
												</div>
												<div class="cad_text" id="cad_input_container_muestra" style="display:none">
													{!! armarInputCreateEditOT('cad_id', 'CAD:', 'text',$errors, $ot, 'form-control', '', '') !!}
												</div>
												<div class="cad_select" id="cad_select_container_muestra">
													...
													<div class="form-group form-row ">
														...
														<div class="col">
															<select name="destinatarios_id[]" id="destinatarios_id" class="form-control form-control-sm" multiple title="Selecciona..." data-selected-text-format="count > 1" data-actions-box="false">
																{!! optionsSelectArrayfilterMultiple([2=>"Retira Diseñador VB",1=>"Retira Ventas VB",4=>"Envío Cliente VB",5=>"Retira Diseñador Revisión",3=>"Envío Laboratorio"],'destinatarios_id') !!}
															</select>
														</div>
													</div>
												</div>
...
<div class="modal fade" id="modalMuestrasExcel" tabindex="-1" role="dialog" aria-labelledby="modalMuestrasExcelLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <form id="formMuestrasExcel"
          class="modal-content"
          method="POST"
          action="{{ route('guardar-muestra-masiva') }}"
          enctype="multipart/form-data">
      @csrf

      <div class="modal-header">
        <h5 class="modal-title" id="modalMuestrasExcelLabel">Subir y validar Excel de muestras</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Cerrar"><span>&times;</span></button>
      </div>

      <div class="modal-body">
        {{-- Hidden con el WorkOrder --}}
        <input type="hidden" name="ot_muestra" id="ot_muestra" value="{{ $ot->id ?? ($ot->id ?? '') }}">
        <input type="hidden" name="validacion_estado" id="validacion_estado" value="0">

        <div class="form-group form-row">
          <div class="col-9">
            <div class="form-group form-row align-items-center">
              <label class="col-auto col-form-label text-right mb-0">Archivo: </label>
              <input type="file" class="form-control-file" id="archivo_excel" name="licitacion_file" accept=".xls,.xlsx" required>
            </div>
          </div>
        </div>
        ...
        <div class="form-group form-row">
          <div class="col-6">
            {!! armarInputCreateEditOT(
                ...
                'text',
                $errors,
                isset($workOrder) ? $workOrder : (isset($ot) ? $ot : null),
                'form-control',
                'readonly',
                '',
            ) !!}
          ...
          <div class="col-3">
            <button type="button" class="btn btn-secondary" id="btnValidarExcel" onclick="validarArchivo()">Validar Excel</button>
          </div>
        </div>

        <div id="resultado_validacion" class="mt-3" style="display:none;">
          <div class="alert" id="resultado_validacion_alert"></div>
          <div id="resultado_validacion_detalles"></div>
        </div>
      </div>

      <div class="modal-footer">
        <button type="button" class="btn btn-outline-secondary" data-dismiss="modal">Cerrar</button>
        <button type="submit" class="btn btn-primary" style="display: none;" id="btnGuardarMuestras">Guardar</button>
      </div>
    </form>
  </div>
</div>
...
document.getElementById('archivo_excel').addEventListener('change', function () {
  $('#validacion_excel').val('');
    $('#validacion_estado').val(0);
   $('#btnGuardarMuestras').attr('style', 'display: none;');
});
        ...
        fetch('{{ route('validarExcel') }}', {
            ...
            .then(data => {
                if (data.validado) {
                    ...
                    $('#validacion_estado').val('1');
                    $('#validacion_excel').val('Validado');
                    $('#btnGuardarMuestras').removeAttr('style');
                } else {
                    data.errores.forEach(error => {
                        toastr.error(error.mensaje);
                    });
                    $('#validacion_excel').val('Archivo No Válido');
                    $('#validacion_estado').val(0);
                    $('#btnGuardarMuestras').attr('style', 'display: none;');
                }
            })
