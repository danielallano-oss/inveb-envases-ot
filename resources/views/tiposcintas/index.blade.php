...
@section('content')
<!-- Titulo -->
<h1 class="page-title">Mantenedor de Tipos Cintas
  <a href="{{ route('mantenedores.tipos-cintas.create') }}" class="btn btn-primary rounded-pill ml-3 px-5">Crear</a>
</h1>

<!-- Filtros -->
<form id="filtros" class="py-3" action="{{ route('mantenedores.tipos-cintas.list') }}" method="get" enctype="multipart/form-data">
  @csrf
  <div class="form-row">
    <div class="col-sm-12 col-md-3 col-lg-3">
      <div class="form-group">
        ...
        <select name="id[]" id="id" class="form-control form-control-sm" multiple data-live-search="true" title="Seleccionar..." data-selected-text-format="count > 1">
          {!! optionsSelectObjetfilterMultiple($tiposcintas_filter,'id',['descripcion','codigo'],'') !!}
        </select>
      </div>
    </div>
...
<div class="container-table mt-3 bg-white border px-2">
  <table class="table table-status table-hover actions states">
    ...
    <tbody>
      @foreach($tiposcintas as $tipocinta)
      <tr class="{{ $tipocinta->active == 1 ? '' : 'text-muted' }}">
        <td>{{ $tipocinta->descripcion }}</td>
        <td>{{ $tipocinta->codigo }}</td>
        <td>
          @if($tipocinta->active == 1) Activo
          @else Inactivo
          @endif
        </td>
        <td>
          <a href="{{route('mantenedores.tipos-cintas.edit', $tipocinta->id)}}">
            <div class="material-icons md-14" data-toggle="tooltip" title="Editar">edit</div>
          </a>
          @if($tipocinta->active == 0 )
          <form method="POST" action="{{ route('mantenedores.tipos-cintas.active', $tipocinta->id) }}" style="display: inline;">
            @method('put')
            @csrf
          ...
          @else
          <form method="POST" id="form_{{ $tipocinta->id }}" action="{{ route('mantenedores.tipos-cintas.inactive', $tipocinta->id) }}" style="display: inline;">
            @method('put')
            @csrf
...
<!-- Paginacion -->
<nav class="mt-3">
  {!! $tiposcintas->appends(request()->query())->links('pagination::bootstrap-4') !!}
</nav>
@endsection
